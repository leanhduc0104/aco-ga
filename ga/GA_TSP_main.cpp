...
#define IN "input22.txt"
#define OUT "ga_out_22.txt"
...
void get_fromFile(){
    ...
    infile.open(IN);
    infile >> n;
    for(int i = 0; i < n; i++) infile >> points[i][0] >> points[i][1];
    infile.close();
    Calc_Distance();
...
void insertBinarySearch(vector<int>& child, double total_cost){
    int imin = 0;
...
{
    initialpopulation();
    if(real_size_population == 0) return;
    for(int i = 0; i < generations; i++)
    {
...
int main() {
    ...
    cout << "Time for to run the genetic algorithm: " << ((double )(end_time - begin_time))/CLOCKS_PER_SEC<<" second.";
    ofstream outfile;
    outfile.open(OUT);
    outfile << "Time for to run the genetic algorithm: " << ((double )(end_time - begin_time))/CLOCKS_PER_SEC<<" second.\n";
    outfile << " Minimum Cost: " << population[0].second << '\n'; 
